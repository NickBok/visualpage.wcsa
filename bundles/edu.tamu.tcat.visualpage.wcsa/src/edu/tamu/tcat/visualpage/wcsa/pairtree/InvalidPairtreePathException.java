package edu.tamu.tcat.visualpage.wcsa.pairtree;

public class InvalidPairtreePathException extends Exception
{

   public InvalidPairtreePathException()
   {
      // TODO Auto-generated constructor stub
   }

   public InvalidPairtreePathException(String message)
   {
      super(message);
      // TODO Auto-generated constructor stub
   }

   public InvalidPairtreePathException(Throwable cause)
   {
      super(cause);
      // TODO Auto-generated constructor stub
   }

   public InvalidPairtreePathException(String message, Throwable cause)
   {
      super(message, cause);
      // TODO Auto-generated constructor stub
   }

   public InvalidPairtreePathException(String message, Throwable cause, boolean enableSuppression, boolean writableStackTrace)
   {
      super(message, cause, enableSuppression, writableStackTrace);
      // TODO Auto-generated constructor stub
   }

}
